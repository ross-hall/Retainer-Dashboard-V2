...
- ⚠️ **Migration status — verified live against the DB on 2026-08-12** (this list had drifted badly out of date before that; if you're unsure, probe the live schema rather than trusting prose):
  ...
  - **New, not yet run:** `outputs/v27_dashboard_zones.sql` — adds `zone` + `client_id` to `rs_stage_categories`, `embed` to `rs_stage_links`, and three `dash_*_url` columns to `rs_clients`. The app degrades gracefully without it (console warns "Dashboard zones not enabled", falls back to the pre-v27 scope-only category query, and the category modal toasts "Run migration v27…" if you try to save a zone).
...
- `outputs/v25_checklist_templates.sql` has not been run yet — Settings > "Checklist templates" degrades gracefully (shows "run migration v25") and "Start from" when creating a checklist only offers "Blank" until `rs_checklist_templates`/`rs_checklist_template_items` exist.
- `outputs/v26_hide_stage_date.sql` has not been run yet — the public-dashboard admin's "Hide"/"Show" button next to a stage's date fails (toast "Run migration v26 to enable hiding dates") until `rs_project_stages.hide_due_date_from_client` exists.
...
- v27: rs_stage_categories.zone + .client_id, rs_stage_links.embed, rs_clients.dash_booking_url/dash_request_url/dash_message_url (public dashboard 3-zone restructure) — **not yet run**, do this in the Supabase SQL editor when convenient. Visually a no-op: everything defaults to zone `'general'`.
