...
- ⚠️ **Pending manual steps:** `outputs/v19_add_animator.sql`, `outputs/v20_anim_cell_statuses.sql`, `outputs/v22_internal_task_multi_assignee.sql`, `outputs/v24_tools.sql`, `outputs/v25_checklist_templates.sql`, and `outputs/v26_hide_stage_date.sql` have not been run yet — Animator assignments won't persist server-side, pipeline cell statuses aren't editable, the Internal Tasks assignee picker won't let you select more than one person, the Tools page degrades gracefully (toast "run migration v24") until `rs_tools` exists, the Settings > "Checklist templates" section degrades gracefully (toast "run migration v25") until `rs_checklist_templates`/`rs_checklist_template_items` exist (until then, "Start from" when creating a checklist only offers "Blank"), and the public-dashboard admin's "Hide"/"Show" button next to a stage's date will fail (toast "Run migration v26 to enable hiding dates") until `rs_project_stages.hide_due_date_from_client` exists. `outputs/v21_internal_tasks.sql` **has been run** — Internal Tasks and Departments are live. `outputs/v23_qa_checklist.sql` **has been run** (confirmed by a live `42501` RLS error on checklist creation, which only happens once the table already exists) but shipped with RLS still enabled; `outputs/v23b_checklists_rls_fix.sql` fixes that — **run this too if you haven't already**, otherwise checklist creation will keep failing with "Could not create checklist". Note: `outputs/v23_qa_checklist.sql` was rewritten mid-session before ever being run the first time — it originally added a `qa_checklist` column to `rs_project_types` + a flat `rs_project_qa_items` table, but was replaced outright with the current `rs_checklists`/`rs_checklist_items` design once the user clarified they wanted standalone, multi-per-project checklists rather than one type-templated list. If you see references to `qa_checklist`/`rs_project_qa_items` anywhere stale, they're leftover from that abandoned draft.
...
- v25: rs_checklist_templates + rs_checklist_template_items (editable checklist templates, managed from Settings; seeds Website/Branding/Animation/Deck starters) — **not yet run**, do this in the Supabase SQL editor when convenient
- v26: rs_project_stages.hide_due_date_from_client (admin can hide a stage's date from the public client dashboard) — **not yet run**, do this in the Supabase SQL editor when convenient
